...
```{r}
#want: each row contain country_year_variable
d |> select(country_name, year)
d_subset <- d |> select(country_name, year, starts_with("e_"))
d_subset_long <- d_subset |> pivot_longer(cols = starts_with("e_"))
...
```{r}
# Want: Each row contains country-year-variable
d |> select(country_name, year)
d_subset <- d |> select(country_name, year, starts_with("e_"))
d_subset_long <-
...
```{r}
#Combines data frame d and data frame d_VdemIndex together
d_stack <- bind_cols(d, d_VdemIndex)
names(d_stack)
```

```{r}
d_1945_2022 <- bind_rows(d, d_Vdem1945)
...
d_1945_1983 <- read_csv()
```

```{r}
d_1945_2002 <- bind_rows(d, d_1945_1983)
d_1945_2002 |> select(year)|> distinct() |> arrange(year)
```
