...
d <- readRDS("Notes/Lec_07/data/wealth_and_health.rds")
d |> print(n = 3)
```
...
d <- d |>
  mutate(
    region = case_match(
      ...
      .default = NA))
table(d$region)
#recode according the vdem codebook
```
...
#verticalize the text in bar chats
```
...
#exchange vertical and horizontal contents
```
...
```{r, fig.height=4}
d |> group_by(region) |> summarise(n_obs = n()) |>
  ggplot(aes(y = reorder(region, n_obs), x = n_obs)) + geom_bar(stat = "identity") +   #order region
...
d <- d |>
  mutate(region_higher = case_match(
    region,
    c("Western Europe", "Northern Europe", "Southern Europe", "Eastern Europe") ~ "Europe", #recode as Europe
...
d |> group_by(region_higher) |> summarise(n_obs = n()) |>
  ggplot(aes(y = reorder(region_higher, n_obs), x = n_obs)) + geom_bar(stat = "identity") +   #identity refers to the original 
  labs(y = "Region", x = "Count", title = "N. Country-Year by Region")  
```
...
`{r} #wrong figure} d |> group_by(region_higher, democracy_binary) |> summarise(n_obs = n()) |>   ggplot(aes(x = n_obs, y = region_higher, fill = democracy_binary)) + #fill:want them in different color   geom_bar(stat = "identity", position = "stack") +   labs(y = "Region", x = "Count", title = "N. Country-Year by Region and Regime Type", fill = "Democracy")`
