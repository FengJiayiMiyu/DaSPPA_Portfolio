...
#saveRDS:recommended, nothing will lose
```
...
d_pop_overlap <- d |> select(country_text_id, year, starts_with("population_")) |> 
  drop_na() #remove rolls that has at least one missing value
print(d_pop_overlap, n = 3)
unique(d_pop_overlap$year)
cor(d_pop_overlap$population_ClioInfra, d_pop_overlap$population_WorldBank) #0.9997128 highly matched
```
...
d <- d |> rowwise() |> #rowwise:extreme version of groupby, take the mean of each role
...
d |> 
  ggplot(aes(x = gdppc, y = life_expectancy)) + #set X and Y
  geom_point() +                                
  labs(x = "GDP per capita", y = "Life Expectancy",              #add labels
...
# Save plot as a PDF file in your folder (recommended)
...
```{r, warning=FALSE, fig.width=9, fig.height=4}
p_all + theme_bw()  #black and white
```
...
```{r, warning=FALSE, fig.width=9, fig.height=4}
p_all + theme_classic()   #remove lines
```
